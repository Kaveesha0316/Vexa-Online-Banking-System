...
<%@ page import="com.example.ee.core.service.AuditService" %>
<%@ page import="com.example.ee.core.model.Customer" %>
<%@ page import="com.example.ee.core.model.AuditLog" %>
...
<%
    try {
        ...
        AuditService auditService = (AuditService) ic.lookup("com.example.ee.core.service.AuditService");

        Customer admin = (Customer) request.getSession().getAttribute("admin");

        List<AuditLog> auditLogList = auditService.findAllAuditLog();

        pageContext.setAttribute("admin", admin);
        pageContext.setAttribute("auditLogList",auditLogList);
    ...
    <div class="table-container">
        <table class="customers-table">
            ...
            <c:forEach var="auditLog" items="${auditLogList}">
                <tr data-nic="${auditLog.username}">
                    <td>${auditLog.username }</td>
                    <td>${auditLog.action}</td>
                    <td>${auditLog.details}</td>
                    <td>Rs.${auditLog.amount}</td>
                    <td><fmt:formatDate value="${auditLog.timestamp}" pattern="MMM d, yyyy hh:mm:ss a" /></td>
                </tr>
            </c:forEach>
